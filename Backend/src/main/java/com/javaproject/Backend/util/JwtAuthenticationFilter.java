...
public class JwtAuthenticationFilter implements Filter {
    ...
    public void doFilter(jakarta.servlet.ServletRequest servletRequest, jakarta.servlet.ServletResponse servletResponse,
        ...
        // 1. Ép kiểu (Casting): Chuyển đổi đối tượng chung sang
        // HttpServletRequest/Response để truy cập HTTP header
        ...
        if (token != null && jwtUtils.validateJwtToken(token)) {
            ...
            // Đây là đối tượng chuẩn mà Spring Security sử dụng để đại diện cho người dùng
            // đã xác thực.
            UsernamePasswordAuthenticationToken auth = new UsernamePasswordAuthenticationToken(
                    ...
                    Collections.emptyList() // Authorities (Quyền hạn): Đặt là danh sách rỗng (có thể thay bằng
                                            // roles/permissions thực tế)
            );
            ...
            // Đây là bước quan trọng nhất: cho Spring Security biết rằng người dùng hiện
            // tại đã được xác thực thành công.
            SecurityContextHolder.getContext().setAuthentication(auth);

            // 4e. (Tùy chọn) Gắn userId vào Request Attribute
            // Giúp các Controller/Service tiếp theo có thể dễ dàng lấy userId mà không cần
            // đọc lại token.
