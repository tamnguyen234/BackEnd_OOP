...
public class JwtUtils {
    // Khóa Bí Mật (Secret Key): Dùng để ký (sign) token, đảm bảo tính toàn vẹn (integrity).
    ...
    return Jwts.builder()
            ...
            .setExpiration( // Thiết lập thời gian token hết hạn (exp - Expiration)
                new Date(System.currentTimeMillis() // trả về thời gian hiện tại của hệ thống (current time) dưới dạng một số nguyên lớn
    ...
    public boolean validateJwtToken(String token) {
        try {
            ...
            // Bắt mọi ngoại lệ liên quan đến JWT (ExpiredJwtException, SignatureException, v.v.)
    ...
    public Long getUserIdFromToken(String token) {
        ...
        // Xử lý kiểu dữ liệu:
        // Thư viện JWT có thể đọc giá trị số nguyên (ví dụ: Long) từ JSON dưới dạng Integer, 
