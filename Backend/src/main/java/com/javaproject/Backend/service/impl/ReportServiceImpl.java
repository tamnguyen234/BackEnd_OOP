...
public class ReportServiceImpl implements ReportService {
    ...
    public List<ExpenseReportRow> generateExpenseReportForCurrentUser(ReportRequest request) {
        ...
        Map<Long, BigDecimal> budgetMap = budgets.stream()
                ...
                .filter(b -> "Chi tiêu".equalsIgnoreCase(b.getCategory().getType())) // chỉ budget của chi tiêu
        ...
        for (Long catId : allCategoryIds) {
            ...
            String catName = budgets.stream()
                    .filter(b -> b.getCategory() != null && b.getCategory().getCategoryId().equals(catId))
                    .map(b -> b.getCategory().getName())
                    .findFirst()
                    .orElse(
                            expenses.stream()
                                    .filter(e -> e.getCategory() != null
                                            && e.getCategory().getCategoryId().equals(catId))
